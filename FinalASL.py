...
def Game():
    ...
    #Label(master, text="First Name:").grid(row=2,column=0, sticky=W)
    #Entry(master, textvar = ent).grid(row=2,column=1, sticky=W)
    Button(master, text='Learn', command = lambda: quit()).grid(row=3, column=0, pady=4)
    Button(master, text='Play', command = lambda: Play()).grid(row=3, column=2, pady=4)
    master.protocol("Continue", quit)
    master.mainloop()
...
def Play():
    print('ya')
    quit()
    #os.system('"python /Users/kevingottfried/Documents/CS_228/Final/final/asl.py"')
    subprocess.Popen("python /Users/kevingottfried/Documents/CS_228/Final/final/asl.py",shell=True )
    sys.exit()
    return
def Dict(ent):
    global userName, loginAttempt
    userName = str(ent.get())
    print 'hi'
    print userName
...
def PlotAll():
    ...
    print recounts
    i = 0
    print len(nums), len(times), len(counts), len(recounts)
    print times[8]
    
                
    area = [10*4 ** n for n in counts]
    #plt.subplot(1,1,1)
    
    plt.scatter(nums, times, s=area, c='g', alpha=0.5)
    plt.xlabel('ASL digits Attempted')
    plt.ylabel('Avg Elapsed Time')
    plt.xlim(-2,10)
    plt.title('Average Elapsed Time of a digit for all users')
    plt.grid()
    plt.pause(10)
    plt.draw()
